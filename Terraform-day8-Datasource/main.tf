...
data "aws_subnet" "selected" {
    filter {
        name   = "tag:Name"
        values = ["mysubnet1"] #fetch the subnet with the tag Name=subnet-1
    }
}
data "aws_security_group" "selected" {
    filter {
        name   = "group-name"
        values = ["my-security-group"] #fetch the security group with the tag group Name=my-security-group
    }
}
